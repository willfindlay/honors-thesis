...
### System Call Sequencing Through Lookahead Pairs
...
The daemon's final task is to automatically save profiles
...
profile for each executable on the system. It does this by maintaining
a hashmap of profiles, hashed by a unique per-executable ID; this ID
is a 64-bit unsigned integer which is calculated as a unique combination
...
executable, as well as a sparse two-dimensional away of lookahead pairs to
keep track of system call sequences. Each entry in this array consists of an
8-bit integer, with the $i^\text{th}$ bit corresponding to a previously observed
distance of $i$ between the two calls.
\autoref{ebph-profile-struct} presents a simplified definition for the ebpH profile struct.
...
lookahead pairs according to the process' most recent system calls.
...
struct ebpH_process
{
    long seq[8];  /* Remember 8 most recent system calls in order */
...
The eight most recent system calls made by each process are stored in its respective
...
\hline
\hline
\texttt{execve} & Execute a program & (Re)associate a process with a profile, creating the profile if necessary\\
\hline
...
#### Profile Creation and Association with Execve
...
#### Reaping Processes with Exit, Exit\_group, and Signals
...
finished with them we ensure that: a) the map never fills up and; b) the map does not consume
...
In order to accomplish this, we simply instrument tracepoints for `fork`, `vfork`, and `clone`
...
Once ebpH has decided that a pr
