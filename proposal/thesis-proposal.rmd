...
IDS can be broadly classified into several categories based on data collection, detection
...
here. However, this should present a good enough overview to understand IDS in
the context of ebpH.
}
\label{ids-overview}
\end{figure}
...
Network-based IDS examine network traffic and analyze it to detect
attacks or anomalies. In contrast, host-based attacks analyze the state
or the local system [@kemmerer02; @soma02].

Responses can vary quite a bit based on the system, but can be classified into
two main categories: alerts and counter-attacks. Systems can either choose to alert
and administrator about the potential issue, or choose to mount counter-measures
...
Implementing this functionality in ebpH is a topic for future work (refer to \autoref{improvements-to-ebph}).
...
design choices, rationale therein. Using the same IDS categories from the previous
...
The concept of homeostasis in pH is inspired by the biological process
of the same name [@soma02].

TODO: come back here when more inspired
...
<!-- TODO: desribe how we also copy sequences on fork -->
...
5) The verifier tends to err on the side of caution and will produce false positives with alarming frequency.
...
work (see \autoref{improvements-to-ebph}).
...
From subproblem (4), we have the obvious issue that loops need to "simple" enough for the eBPF
verifier to reason about them. Loops need to have an induction variable that can be shown to
invariably lead to termination. For example, a loop like the one shown in \autoref{invalid-loop}
would be prohibited.

\begin{lstlisting}[float={ht}, language=c, label={invalid-loop}, caption={[An invalid bounded loop in eBPF.]An invalid bounded loop in eBPF.
Although it may be obvious to us that it will eventually terminate, the induction variable is not well-defined enough for the verifier
to be confident about its termination.}]
int j = 50;
/* An invalid eBPF bounded loop */
for (int i = 0; i < j; i++)
{
    /* Increase j by 1 every 3 iterations */
    if (i % 3 == 0)
        j++;
}
\end{lstlisting}

Loops may also be rejected by the verifier for other reasons. For example, loops that
have entrypoints in the middle of iteration will also be flagged due to the verifier's
inability to correctly identify the loop structure [@corbet18]. Ultimately, designing
verifiable loops in eBPF can be a tricky process with many pitfalls, but having support
for loops at all greatly increases the potential complexity and power of eBPF programs like ebpH.
...
untrusted program is a difficult problem, and concessions need to be made in order for such guarantees
...
Fortunately, well-contracted eBPF do not often suffer from false positives, particularly as one learns
the nuances of how the verifier works and how to coax it into accepting programs
...
machines under a variety of workloads. Additional testing will be done on virtual machines
to simulate systems with lower end hardware specifications. \autoref{testing-specifications}
summarizes the currently planned systems as their relevant specifications.
...
\hline
\textbf{System} & \textbf{CPU Speed} & \textbf{RAM} & \textbf{Description}\\
\hline
\hline
arch & test & test & test\\
\hline
