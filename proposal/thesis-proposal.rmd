...
through an even newer subset of eBPF, known as *XDP* or *Express Data Path*, we
can inspect, modify, redirect, and even drop packets entirely before
they even reach the main kernel network stack. <!-- TODO: add a citation for XDP here -->
\autoref{ebpf-use-cases} provides
a high level overview of these use cases and the corresponding eBPF instrumentation
required.

\begin{figure}
\begin{center}
\includegraphics[keepaspectratio, height=4.5in]{../figures/eBPF-use-cases.png}
\end{center}
...
While the verifier does limit what is possible (eBPF is **not** Turing complete
<!-- TODO: cite here -->), it is constantly being
improved <!-- TODO: cite some kernel patches here --> and
even in its current state, eBPF is quite feature-rich and capable. The verifier will
be discussed in further detail in \autoref{verifier-section}.

eBPF's superior performance can be attributed to several factors;
on supported architectures,\footnote{TODO: which supported architectures?}
...
to insert an eBPF program into the kernel <!-- TODO: cite here -->.
