...
\makeatother
\subtitle{COMP4906 Honours Thesis Proposal}
...
The \textbf{ebpH Daemon} (\emph{ebpHD}) is responsible for initially
...
the \textbf{ebpH GUI}. The GUI performs the same basic functions as the
...
userspace, particularly since buffering data significantly reduces the
number of required context switches.
...
\hypertarget{the-ebph-gui}{%
\subsubsection{The ebpH GUI}\label{the-ebph-gui}}

The ebpH GUI (hereafter referred to as the GUI) provides a graphical
user interface for interacting with the daemon. This GUI is still a work
in progress and will be improved considerably during the testing and
iteration phase (see \autoref{methodology-and-future-work}). In its
current incarnation, the GUI can be used to inspect profiles, examine
the overall state of ebpH, and check the ebpH logs. It can also be used
to issue rudimentary commands such as profile deletion. Future versions
of the GUI will include more commands for controlling the state of ebpH,
as well as increased system visibility and more information about
processes and profiles. \autoref{early-gui} depicts an early version of
the GUI.

\begin{figure}
\centering
\includegraphics{../figures/early_gui.png}
\caption{\label{early-gui}A screenshot of an early version of the ebpH
GUI.}
\end{figure}
...
\hypertarget{profile-creation-and-association-with-execve-and-execveat}{%
\paragraph{\texorpdfstring{Profile Creation and Association with
\texttt{execve} and
\texttt{execveat}}{Profile Creation and Association with execve and execveat}}\label{profile-creation-and-association-with-execve-and-execveat}}
...
order to access the file's inode and filesystem information; without
this, we would be unable to differentiate between two paths that resolve
to a binary with the same name, for example
...
\hypertarget{profile-association-and-sequence-duplication-with-fork-vfork-and-clone}{%
\paragraph{\texorpdfstring{Profile Association and Sequence Duplication
with \texttt{fork}, \texttt{vfork}, and
\texttt{clone}}{Profile Association and Sequence Duplication with fork, vfork, and clone}}\label{profile-association-and-sequence-duplication-with-fork-vfork-and-clone}}
...
same profile as its parent. In order to accomplish this, we instrument
tracepoints for the \passthrough{\lstinline!fork!},
...
overwrite the profile association provided by the initial fork. The
parent's current system call sequence is also copied to the child to
prevent forks from being used to break sequences.

\hypertarget{reaping-processes-with-exit-exit_group-and-signals}{%
\paragraph{\texorpdfstring{Reaping Processes with \texttt{exit},
\texttt{exit\_group}, and
Signals}{Reaping Processes with exit, exit\_group, and Signals}}\label{reaping-processes-with-exit-exit_group-and-signals}}

We use a combination of system call tracepoints and signal handler
kprobes in order to determine when to stop tracing a particular PID.
This is important for a few reasons, primarily due to map size
considerations; by reaping process structs from our map as we are
...
Processes are reaped from ebpH's map whenever it detects an
\passthrough{\lstinline!exit!} or \passthrough{\lstinline!exit\_group!}
system call. Threads are reaped whenever we observe a
\passthrough{\lstinline!SIGTERM!} or \passthrough{\lstinline!SIGKILL!}
signal, the latter of which forms the underlying implementation for
\passthrough{\lstinline!exit\_group!}.
...
quite understandable from the perspective of the verifier. As we have
already seen, guaranteeing the safety of arbitrary untrusted programs is
a difficult problem, and concessions need to be made in order for such
guarantees to be tenable. False positives are unfortunately one of those
concessions. When the verifier rejects code due to a false positive,
there is simply no better solution than to try a different approach.
Fortunately, well-constructed eBPF programs do not often suffer from
false verifier positives, particularly as one learns the nuances of how
the verifier works and how to coax it into accepting programs.
...
\hypertarget{improvements-to-the-ebph-gui}{%
\subsection{Improvements to the ebpH
GUI}\label{improvements-to-the-ebph-gui}}

The GUI has potentially the most room for improvement of the entire ebpH
system. Many of these improvements will be in tandem to those discussed
in previous sections; in particular, system visibility enhancements to
ebpH will almost certainly manifest themselves through the GUI wherein
we will be able to visually present information to the user as they
request it.

Future versions of the GUI will also have more options for inspecting
and making changes to system profiles. Somayaji describes three
important operations in his dissertation \autocite{soma02},
\emph{tolerize}, \emph{sensitize}, and \emph{normalize}, which have not
yet been implemented in ebpH. These options will be added to the GUI
when they are implemented to allow users to have fine grained control
over how ebpH treats the profiles on their system. Additionally, users
should be able to inspect individual profiles and look at their data in
a format that is easy to understand; potentially this could be achieved
via plotting profile data or perhaps presenting the top \emph{n} system
call lookahead pairs by process.

Ultimately, the GUI is perhaps among the most important components of
ebpH, particularly given the usability requirements we have discussed in
previous sections. As such, it is one of the most important factors
controlling the potential future adoption of ebpH, and is therefore
important to get exactly right.
