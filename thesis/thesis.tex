...
observed patterns) or signature-based, which generally refers to
matching known attack patterns to observed data
\autocite{kemmerer02,vanoorschot19}. In general, anomaly-based
approaches cover a wider variety of attacks while signature-based
...
we observe a process making a call to \passthrough{\lstinline!execve!}.
...
assign profiles to processes, which is done by instrumenting the result
of an \passthrough{\lstinline!execve(2)!} system call using the
...
Without this, we would be unable to differentiate between two paths that
resolve to a binary with the same name, for example
...
lookahead pairs. This is important in order to prevent
...
instrument tracepoints for the \passthrough{\lstinline!fork(2)!},
\passthrough{\lstinline!vfork(2)!}, and
\passthrough{\lstinline!clone(2)!} system calls, ensuring that we
associate the child process with the parent's profile, if it exists. If
ebpH detects an \passthrough{\lstinline!execve(2)!} as outlined above,
it will simply overwrite the initial profile association provided by the
fork. The parent's current system call sequence is also copied to the
child to prevent forks from being used to break sequences.
...
(c.f. \autoref{response_automation}).
...
tracepoint, for example \passthrough{\lstinline!sched\_switch!}.
...
\hypertarget{refactoring-the-ebph-gui-and-conducting-a-usability-study}{%
\subsection{Refactoring the ebpH GUI and Conducting a Usability
Study}\label{refactoring-the-ebph-gui-and-conducting-a-usability-study}}
