...
\(600\%\), which is representative of the worst case, but longer system
calls like \passthrough{\lstinline!stat(2)!}, overhead drops to about
\(67\%\). This overhead is more representative of the general case.
...
execution of simple signal handlers is relatively low, around 41\%. This
...
\autoref{bronte_lmbench_process} shows that ebpH imposes at most
negligible overhead on process creation. The actual result shows a
performance \emph{improvement} with ebpH, but this is well within the
margin of error for the test and actual overhead is likely on the order
of less than 1\%.
...
call and imposes a total performance overhead of just 6\%.
...
method is only about 8.5\%, which is acceptable in practice.
...
methods of IPC, pipes and Unix domain stream sockets. In both cases, the
overhead caused by ebpH does not exceed 25\%.
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=\textheight]{../data/bench/bronte-7day/under_3us_times.png}
\caption{\label{bronte_7day_graph}Top 20 most frequent system calls with
base times of under 3\(\mu\)s from the \code{bronte-7day} dataset (after
discarding outliers). Smaller difference in times is better. Standard
error is given as error bars.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth,height=\textheight]{../data/bench/bronte-7day/lmbench_parity_times.png}
\caption{\label{bronte_7day_parity_graph}Selected system calls from the
\code{bronte-7day} dataset, for easy comparison with the \code{lmbench}
data from \autoref{lmbench_section}. Smaller difference in times is
...
\tightlist
\item
  Establishing the viability of eBPF as a method for host-based
...
\item
  The need for a security analysis of ebpH;
\item
